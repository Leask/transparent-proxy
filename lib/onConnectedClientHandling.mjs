...
import { Session, parseHeaders, rebuildHeaders } from './Session.mjs';
import findUpstreamProxy from './findUpstreamProxy.mjs';
import getConnectionOptions from './getConnectionOptions.mjs';
import net from 'net';

import {
    EVENTS, HTTP_BODIES, HTTP_METHODS, HTTP_RESPONSES,
 ...
 * @param logger
 */
export default (clientSocket, bridgedConnections, options, logger) => {
    const {
    ...
    async function prepareTunnel(data, firstHeaderRow, isConnectMethod = false) {
        ...
        const proxyToUse = await findUpstreamProxy(upstream, {
